...
<div class="w-full">
    ...
    <div class="relative rounded-md shadow-sm">
        @if($prefix || $icon)
            <div class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3 text-slate-500 dark:text-slate-400">
                @if($icon)
                    {!! $icon !!}
                @else
                    {{ $prefix }}
        ...
        @if($suffix)
            <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-3 text-slate-500 dark:text-slate-400">
                {{ $suffix }}
            </div>
        @elseif($hasError)
