<div>
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-semibold text-neural-900 dark:text-neural-100">Manage News</h1>
        <x-ui.button wire:navigate href="{{ route('admin.news.create') }}">Create New</x-ui.button>
    </div>
    <x-ui.card>
        ...
        <x-ui.table>
            <x-slot:head>
                <x-ui.table.row>
                    ...
                    <x-ui.table.heading>Published Date</x-ui.table.heading>
            ...
            <x-slot:body>
                ...
                <x-ui.table.row>
                    ...
                    <x-ui.table.cell>{{ $record->category_id ?? 'N/A' }}</x-ui.table.cell>
                    <x-ui.table.cell>{{ $record->status ?? 'N/A' }}</x-ui.table.cell>
                    <x-ui.table.cell>{{ $record->published_at ?? 'N/A' }}</x-ui.table.cell>
                    <x-ui.table.cell class="text-right">
                        <x-ui.button size="sm" variant="secondary" wire:navigate href="{{ route('admin.news.edit', $record) }}">Edit</x-ui.button>
