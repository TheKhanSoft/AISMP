<div>
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-semibold text-neural-900 dark:text-neural-100">Manage ContactMessages</h1>
        <x-ui.button wire:navigate href="{{ route('admin.contact.create') }}">Create New</x-ui.button>
    </div>
    <x-ui.card>
        ...
        <x-ui.table>
            <x-slot:head>
                <x-ui.table.row>
                    ...
                    <x-ui.table.heading>Read</x-ui.table.heading>
            ...
            <x-slot:body>
                ...
                <x-ui.table.row>
                    ...
                    <x-ui.table.cell>{{ $record->email ?? 'N/A' }}</x-ui.table.cell>
                    <x-ui.table.cell>{{ $record->subject ?? 'N/A' }}</x-ui.table.cell>
                    <x-ui.table.cell>{{ $record->is_read ?? 'N/A' }}</x-ui.table.cell>
                    <x-ui.table.cell class="text-right">
                        <x-ui.button size="sm" variant="secondary" wire:navigate href="{{ route('admin.contact.edit', $record) }}">Edit</x-ui.button>
